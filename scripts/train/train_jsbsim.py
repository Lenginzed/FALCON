...
def _scenario_config_name(all_args):
    scenario_config_path = getattr(all_args, "scenario_config_path", None)
    if scenario_config_path:
        return str(Path(scenario_config_path).resolve())
    return all_args.scenario_name


def _scenario_run_label(all_args):
    scenario_config_path = getattr(all_args, "scenario_config_path", None)
    if scenario_config_path:
        return f"external_yaml/{Path(scenario_config_path).stem}"
    return all_args.scenario_name
...
def make_train_env(all_args):
    def get_env_fn(rank):
        def init_env():
            scenario_config_name = _scenario_config_name(all_args)
            if all_args.env_name == "SingleCombat":
                env = SingleCombatEnv(scenario_config_name)
            elif all_args.env_name == "SingleControl":
                env = SingleControlEnv(scenario_config_name)
            elif all_args.env_name == "MultipleCombat":
                env = MultipleCombatEnv(scenario_config_name)
            else:
...
def parse_args(args, parser):
    ...
    group.add_argument('--scenario-config-path', type=str, default=None,
                       help="Optional explicit YAML scenario config path. If omitted, --scenario-name behavior is unchanged.")
...
def main(args):
    ...
    # run dir
    scenario_label = _scenario_run_label(all_args)
    run_dir = Path(os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + "/results") \
        / all_args.env_name / scenario_label / all_args.algorithm_name / all_args.experiment_name
